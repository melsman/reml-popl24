...
(* Modified by Martin Elsman:
 ...
 2. Disable parallel construction of BVH structure.
*)
    ...
    let fun mk _ _ [] = raise Fail "mk_bvh: no nodes"
          ...
          | mk d n xs =
            let val axis = case d mod 3 of 0 => V.x
                ...
                val (left, right) = (do_left(), do_right())
...
type image = { R: int Array.array
             , G: int Array.array
             , B: int Array.array
             , height: int
             , width: int}

fun to_pixels ({R,G,B,height,width}:image) : pixel array =
    Array.tabulate(height*width,
                   fn i => (Array.sub(R,i),
                            Array.sub(G,i),
                            Array.sub(B,i)))

fun image2ppm out (img as {height, width, ...}: image) =
    let fun onPixel (r,g,b) =
            TextIO.output(out,
                          Int.toString r ^ " " ^
                          Int.toString g ^ " " ^
                          Int.toString b ^ "\n")
        val pixels = to_pixels img
...
fun image2ppm6 out (img as {height, width, ...}: image) =
    let
      fun onPixel (r,g,b) =
...
val GRAIN = CommandLineArgs.parseInt "G" 1600

fun render objs width height cam : image =
    let val R = ForkJoin.alloc (height*width) 0
        val G = ForkJoin.alloc (height*width) 0
        val B = ForkJoin.alloc (height*width) 0
        fun pixel l =
            let val i = l mod width
                val j = height - l div width
                val (r,g,b) = colour_to_pixel (trace_ray objs width height cam j i)
            in Array.update (R,l,r)
             ; Array.update (G,l,g)
             ; Array.update (B,l,b)
            end
